...
import { setEmployeeDepartment } from '@/lib/departmentStorage';
...
interface EmployeePanelProps {
  ...
  onLeaveChange?: () => void;
  allDepartments?: string[]; // known departments for the reassignment dropdown
  onDepartmentChange?: () => void; // refresh dashboard data after a reassignment
}
...
export default function EmployeePanel({
  ...
  // Optimistic display value + the pending value awaiting confirmation.
  const [displayDept, setDisplayDept] = useState(employee?.department ?? '');
  ...
  // Reset the dropdown whenever a different employee is opened.
  useEffect(() => {
    setDisplayDept(employee?.department ?? '');
    setPendingDept(null);
  }, [employee?.employeeCode, employee?.officeCode]);

  if (!employee) return null;

  const records = (employee.records || []).sort((a, b) => a.date.localeCompare(b.date));

  const departmentOptions = allDepartments.includes(displayDept)
    ? allDepartments
    : [displayDept, ...allDepartments].filter(Boolean);

  function requestDepartmentChange(newDept: string) {
    if (!newDept || newDept === displayDept) return;
    setPendingDept(newDept);
  }

  function confirmDepartmentChange() {
    if (!employee || !pendingDept) return;
    setEmployeeDepartment(employee.employeeCode, employee.officeCode, pendingDept);
    setDisplayDept(pendingDept);
    setPendingDept(null);
    onDepartmentChange?.();
  }
  ...
  return (
    <>
        ...
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-800 flex-shrink-0">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {readOnly ? (
                <span className="text-slate-400 text-xs">{displayDept}</span>
              ) : (
                <select
                  value={displayDept}
                  onChange={(e) => requestDepartmentChange(e.target.value)}
                  title="Change department"
                  className="bg-slate-800 border border-slate-700 rounded-md px-1.5 py-0.5 text-slate-300 text-xs focus:outline-none focus:border-blue-500 hover:border-slate-500 transition-colors"
                >
      ...
      {pendingDept && (
        <ConfirmDialog
          title="Change Department"
          message={`Move ${employee.employeeName} from "${displayDept}" to "${pendingDept}"? This updates how they're grouped in KPIs, charts and department filters going forward.`}
