...
// D2-3: this is the same form that lived at
// app/leave/admin/leave/RecordLeaveForm.tsx, moved here so it can be
// reused inside RecordLeaveDrawer (launchable from the grid or the
// Employee Modal) instead of only living on its own page. Two changes
// from the original:
//   1. The employee to preselect now comes from a `presetEmployeeId` prop
//      (set by whichever card/modal launched the drawer) instead of a
//      `?employee=` URL query param — there's no longer a page/URL for
//      this to live in, so no useSearchParams / Suspense boundary needed.
//   2. `onSuccess` is now called after a successful save so the drawer
//      can trigger a balance refresh and schedule its own auto-close,
//      while this form still shows the confirmation/policy notes inline
//      exactly as it did before.
// Validation, the POST to /api/leave/employees/requests, half-day
...
export default function RecordLeaveForm({
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        // Reuses the existing GET /api/leave/employees list endpoint
        // (already used by AddEmployeeForm's dropdowns) instead of the
        // page-scoped /leave/admin/leave/employees route that went away
