  ...
  return (
    <>
      <button
        ...
        className="border border-[var(--border)] hover:border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-primary)] p-2 rounded-lg transition-colors"
      >
        <Info size={16} />
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="scroll-thin bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-5 text-sm text-[var(--text-muted)]">
              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Leave Types & Balances</h4>
                {loading && <p className="text-[var(--text-muted)] text-xs">Loading…</p>}
                {error && <p className="text-red-700 dark:text-red-300 text-xs">{error}</p>}
                {leaveTypes.length > 0 && (
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {leaveTypes.map((lt) => (
                        <tr key={lt.code} className="border-b border-[var(--border)]/60 last:border-0">
                          <td className="py-1 text-[var(--text-primary)]">{lt.display_name} ({lt.code})</td>
              ...
              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Approval Flow</h4>
                <p className="text-xs">
              ...
              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Attendance Rules</h4>
                <ul className="text-xs list-disc pl-4 space-y-1">
                  <li><span className="text-[var(--text-primary)]">Present</span> — normal punch-in/out for the day, worked minutes above the half-day threshold.</li>
                  <li><span className="text-[var(--text-primary)]">Unmarked Leave</span> — no valid punch activity for the day, and no approved leave, WFH, holiday, or weekly-off covering it.</li>
                  <li><span className="text-[var(--text-primary)]">Late Coming / Early Leaving</span> — in-time after or out-time before the office's standard shift window; tracked as an attendance note on the record, not a separate leave/half-day category by itself.</li>
                </ul>
              </section>

              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Half Day Rules</h4>
                <p className="text-xs">
                  If the gap between an employee's first and last punch for the day is 5 hours or less, the day is
                  <span className="text-[var(--text-primary)]"> not</span> automatically left as unmarked leave — it surfaces under "Possible
              ...
              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Missed Punch Rules</h4>
                <p className="text-xs">
                  ...
                  <span className="text-[var(--text-primary)]"> not</span> create a leave request and does not touch any leave
                  balance.
                </p>
              </section>

              <section>
                <h4 className="text-[var(--text-primary)] font-medium mb-2">Working Hours & Exemptions</h4>
                <p className="text-xs">
