...
function NavItemButton({ item, active, collapsed }: { item: NavItem; active: boolean; collapsed?: boolean }) {
  ...
  const inner = (
    <>
      ...
      {item.kind === 'action' && !!item.badge && !collapsed && (
        <span
  ...
  const wrapClass = collapsed ? `relative ${baseClass}` : baseClass;

  if (item.kind === 'link') {
    return (
      <Link href={item.href} className={wrapClass} title={collapsed ? item.label : undefined}>
        {inner}
      </Link>
    );
  }
...
export default function DashboardShell({
  ...
  // Collapsed = icon-only rail. Persisted so it stays how you left it
  // across visits, same idea as the theme toggle.
    ...
    <div className="h-screen bg-[var(--bg-surface)] text-[var(--text-primary)] md:flex md:overflow-hidden">
      ...
      >
        <div className={`pt-5 pb-4 ${collapsed ? 'px-2' : 'px-4'}`}>
          ...
          {!collapsed && variant !== 'hr' && recordCount !== undefined && (
            ...
            <div key={group.label}>
              ...
              <div className="space-y-0.5">
                {group.items.map((item) => (
                  <div key={item.id}>
                    <NavItemButton item={item} active={item.kind === 'scroll' && activeId === item.id} collapsed={collapsed} />
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className={`border-t border-[var(--border)] py-3 space-y-2 ${collapsed ? 'px-2' : 'px-3'}`}>
          ...
          <div className={`flex items-center gap-2 ${collapsed ? 'flex-col' : 'justify-between'}`}>
            {!collapsed && <span className="text-[11px] text-[var(--text-muted)] truncate">{VARIANT_LABEL[variant]}</span>}
            <div className={`flex items-center gap-1.5 shrink-0 ${collapsed ? 'flex-col' : ''}`}>
